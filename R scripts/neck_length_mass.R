...
long.p <- data.frame(matrix(nrow = 23, ncol = 3))
...
make_bins <- function(data, mass_col, trait_col, nbin, boot_reps = 100) {
  ...
  finalize_summary <- function(summ_df, idx_list, type_label) {
    mass_means <- sapply(idx_list, function(idx) mean(data[[mass_col]][idx], na.rm = TRUE))
    
    base_df <- data.frame(
      subset_name = paste0("bin", seq_along(idx_list)),
      bin = seq_along(idx_list),
      mass_mean = mass_means,
      type = type_label,
...
bin.plot <- 
  ...
  scale_x_continuous(
    ...
    labels = paste0(combined$type, combined$bin, "\n", 
                    round(10^(combined$mass_mean), 2), "kg")  
  ) +
  labs(
    x = "bin order (sorted by mean mass)",
